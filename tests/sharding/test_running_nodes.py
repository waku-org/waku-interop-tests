...
class TestRunningNodesAutosharding(StepsSharding):
    def test_same_cluster_same_content_topic(self):
        ...
        self.check_published_message_reaches_relay_peer(self.test_message, content_topic=self.test_content_topic)
...
class TestRunningNodesStaticSharding(StepsSharding):
    def test_same_cluster_same_content_topic(self):
        ...
        self.check_published_message_reaches_relay_peer(self.test_message, pubsub_topic=self.test_pubsub_topic)

    def test_same_cluster_different_content_topic(self):
        self.setup_main_relay_nodes(cluster_id=2)
        self.subscribe_first_relay_node(pubsub_topics=[self.test_pubsub_topic])
        self.subscribe_second_relay_node(pubsub_topics=["/waku/2/rs/2/4"])
        try:
            self.check_published_message_reaches_relay_peer(self.test_message, pubsub_topic=self.test_pubsub_topic)
            raise AssertionError("Publish on different content topic worked!!!")
        except Exception as ex:
